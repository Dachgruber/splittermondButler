...
public class RollTemplate {
	...
	/**
	 * Puzzling of the embed, display the roll author, the results, the dice used
	 * etc
	 *
	...
	public EmbedBuilder buildRollEmbed(GuildMessageReceivedEvent event, Roll rollEvent) {
		...
		this.embed.setTitle(rollEvent.getDiceAmount() + " W" + rollEvent.getDiceSize() + "🎲");

		this.embed.setDescription(event.getAuthor().getAsMention() + " rolled " + rollEvent.getDiceAmount() + " W"
				+ rollEvent.getDiceSize() + "\n" + "resulting in " + "**" + rollEvent.getResult() + "**");

		this.embed.addField("The Results are:", this.makeListToString(rollEvent.getResultField()), false);
		// embed.addField("With a Sum of", Integer.toString(resultInteger), false);

		if (this.testIfGood(rollEvent.getResultField(), rollEvent.getDiceSize()))
			this.embed.addField("Critical Success!", "", false);
		if (this.testIfBad(rollEvent.getResultField()))
			this.embed.addField("Critical Failure!", "", false);
	...
	public EmbedBuilder buildPrivateRollEmbed(GuildMessageReceivedEvent currentEvent, Roll rollEvent) {
		this.embed = this.buildRollEmbed(currentEvent, rollEvent);
		this.embed.setTitle("GMRoll: " + rollEvent.getDiceAmount() + " W" + rollEvent.getDiceSize() + "🎲");
		this.embed.setDescription(currentEvent.getAuthor().getAsMention() + " rolled secrently"
				+ rollEvent.getDiceAmount() + " W" + rollEvent.getDiceSize() + " to you.\n" + "resulting in " + "**"
				+ rollEvent.getResult() + "**");
		return this.embed;
	}
...
//	}
	...
	/**
	 * checks if the throw is considered a critical success. Only twice the
	 * bestPossible, or one bestPossible and one bestPossible-1 result in critical
	 * success
	 *
	 * @param resultList
	 * @param bestPossible
	...
	private boolean testIfGood(int[] resultList, int bestPossible) {
		...
		if (ArrayUtils.contains(resultList, bestPossible)) {
			...
			// if the num next to bestPossible is included or the bestPossible is included
			// twice, the throw is a success!
	...
	/**
	 * checks if the throw is considered a critical failure. Only twice the
	 * bestPossible, or one bestPossible and one bestPossible-1 result in critical
	 * success
	 *
	 * @param resultList
